...
fn read_msa_header(msa_data: &[u8]) -> Option<MsaHeader> {
    if msa_data.len() < 10 {
        return None;
    }
...
fn decode_msa_to_st(msa_data: &[u8]) -> Option<Vec<u8>> {
    let msa_header = match read_msa_header(msa_data) {
        None => { println!("Invalid MSA header data"); return None; },
        Some(header) => header,
    };
    ...
    for entry in fs::read_dir(src_dir)? {
        ...
        if entry.file_type()?.is_dir() {
            ...
            if ext.to_lowercase() == "msa" {
                ...
                let st_data = match decode_msa_to_st(&msa_data) {
                    None => {
                        println!(
                            "Could not convert {}. Skipping this file.",
                            src_path.display()
                        );
                        continue;
                    },
